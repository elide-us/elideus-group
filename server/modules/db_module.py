...
class DbModule(BaseModule):
  ...
  async def get_renewal(self, recid: int | None) -> dict[str, Any] | None:
    result = await self.run(get_renewal_request(GetRenewalParams(recid=recid)))
  ...
  async def upsert_renewal(self, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    await self.run(upsert_renewal_request(params))
    return params.model_dump()

  async def delete_renewal(self, recid: int) -> dict[str, Any]:
    await self.run(delete_renewal_request(DeleteRenewalParams(recid=recid)))
    return {"recid": recid, "deleted": True}

  async def list_conversations(self, limit: int, offset: int) -> list[dict[str, Any]]:
    res = await self.run(list_summary_request(ListConversationSummaryParams(limit=limit, offset=offset)))
    return list(res.rows or [])

  async def get_conversation_stats(self) -> dict[str, Any]:
    res = await self.run(get_stats_request(ConversationStatsParams()))
    return dict(res.rows[0]) if res.rows else {}

  async def list_conversation_thread(self, thread_id: str) -> list[dict[str, Any]]:
    res = await self.run(list_thread_request(ListThreadParams(thread_id=thread_id)))
    return list(res.rows or [])

  async def delete_conversation_thread(self, thread_id: str) -> int:
    res = await self.run(delete_by_thread_request(DeleteByThreadParams(thread_id=thread_id)))
    return res.rowcount

  async def delete_conversations_before(self, before: str) -> int:
    res = await self.run(delete_before_timestamp_request(DeleteByTimestampParams(before=before)))
    return res.rowcount

  async def list_models_registry(self) -> list[dict[str, Any]]:
    res = await self.run(list_models_request())
    return list(res.rows or [])

  async def upsert_model_registry(self, payload: dict[str, Any]) -> dict[str, Any]:
    params = UpsertModelParams(**payload)
    await self.run(upsert_model_request(params))
    return params.model_dump()
  ...
  async def delete_model_registry(self, recid: int | None = None, name: str | None = None) -> dict[str, Any]:
    ...
    await self.run(delete_model_request(params))
    return params.model_dump()

  async def get_api_providers(self) -> list[str]:
    res = await self.run(get_config_request(ConfigKeyParams(key="ApiProviders")))
  ...
  async def get_user_credits(self, guid: str) -> dict[str, Any]:
    res = await self.run(get_credits_request(GetCreditsParams(guid=guid)))
    return dict(res.rows[0]) if res.rows else {}

  async def get_guild_credits(self, guild_id: str) -> int:
    res = await self.run(get_guild_request(GuildIdParams(guild_id=guild_id)))
    row = res.rows[0] if res.rows else {}
    return int(row.get("element_credits", 0) or 0)
