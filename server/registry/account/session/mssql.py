...
async def create_session_v1(args: dict[str, Any]) -> DBResponse:
  ...
  rotkey = args["rotkey"]
  rotkey_iat = args["rotkey_iat"]
  rotkey_exp = args["rotkey_exp"]
...
async def set_rotkey_v1(args: dict[str, Any]) -> DBResponse:
  ...
  iat = args["iat"]
  exp = args["exp"]
  device_guid = args.get("device_guid")
  sql = """
    UPDATE sd
  ...
  """
  return await run_exec(sql, (rotkey, iat, exp, guid, device_guid, device_guid))
