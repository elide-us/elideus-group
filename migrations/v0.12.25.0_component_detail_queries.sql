-- ============================================================================
-- v0.12.25.0_component_detail_queries
-- Adds get_component_detail and upsert_component data-driven queries
-- for CmsWorkbenchModule property editing.
--
-- Deterministic GUIDs (uuid5):
--   query:cms.components.get_component_detail = 73E3EDC9-1776-5BE7-B144-392A55F9FD45
--   query:cms.components.upsert_component     = 3AEF8758-795C-5053-B7DA-AB4F045579FB
-- ============================================================================
...
FOR JSON PATH, WITHOUT_ARRAY_WRAPPER, INCLUDE_NULL_VALUES;' AS pub_query_text,
    N'Return full detail for a single component.' AS pub_description,
...
WHEN NOT MATCHED THEN
  ...
  VALUES (
    ...
    src.pub_is_parameterized,
    src.pub_parameter_names,
    src.pub_is_active
  );
GO
...
WHERE key_guid = TRY_CAST(? AS UNIQUEIDENTIFIER);' AS pub_query_text,
    N'Update component description and default type.' AS pub_description,
    N'CF85FB11-5981-56B7-8E43-9D453E611D43' AS ref_module_guid,
    1 AS pub_is_parameterized,
