...
const remask = (roles: SystemRolesRoleItem1[]): SystemRolesRoleItem1[] =>
    roles.map((r, idx) => ({
        ...r,
        mask: (1n << BigInt(idx)).toString(),
    }));
...
const SystemRolesPage = (): JSX.Element => {
    ...
    const move = async (index: number, dir: number): Promise<void> => {
        ...
        const remasked = remask(updated);
        setItems(remasked);
        for (const r of remasked) {
            await fetchUpsertRole(r);
        }
        setNotification(true);
    };
