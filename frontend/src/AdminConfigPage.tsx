...
import type { ConfigItem, AdminConfigList1 } from './shared/RpcModels';
import { fetchList, fetchSet, fetchDelete } from './rpc/admin/config';

const AdminConfigPage = (): JSX.Element => {
    const [items, setItems] = useState<ConfigItem[]>([]);
    const [newItem, setNewItem] = useState<ConfigItem>({ key: '', value: '' });

    const load = async (): Promise<void> => {
        try {
            const res: AdminConfigList1 = await fetchList();
            setItems(res.items);
        } catch {
            setItems([]);
        }
