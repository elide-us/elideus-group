...
from rpc.admin.routes.models import AdminRoutesList1, AdminRouteItem, AdminRouteUpdate1, AdminRouteDelete1
...
async def list_routes_v1(request: Request) -> RPCResponse:
  ...
  payload = AdminRoutesList1(routes=routes)
  return RPCResponse(op='urn:admin:routes:list:1', payload=payload, version=1)

async def set_route_v1(rpc_request: RPCRequest, request: Request) -> RPCResponse:
  data = AdminRouteUpdate1(**(rpc_request.payload or {}))
  db: DatabaseModule = request.app.state.database
...
async def delete_route_v1(rpc_request: RPCRequest, request: Request) -> RPCResponse:
  data = AdminRouteDelete1(**(rpc_request.payload or {}))
  db: DatabaseModule = request.app.state.database
