...
async def get_user_profile_v1(rpc_request: RPCRequest, request: Request) -> RPCResponse:
  ...
  result = AdminUserProfile1(
    guid=user.get('guid'),
    displayName=user.get('display_name', ''),
    email=user.get('email', ''),
    credits=user.get('credits'),
    storageUsed=user.get('storage_used'),
    displayEmail=user.get('display_email', False),
    defaultProvider=user.get('provider_name'),
  )
  return RPCResponse(op='urn:admin:users:get_profile:1', payload=result, version=1)
