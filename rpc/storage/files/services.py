...
async def storage_files_get_files_v1(request: Request):
  ...
  files = await module.list_files_by_user(user_guid)
...
async def storage_files_upload_files_v1(request: Request):
  ...
  module: StorageModule = request.app.state.storage
  await module.upload_files(user_guid, data.files)
  return RPCResponse(
    op=rpc_request.op,
...
async def storage_files_delete_files_v1(request: Request):
  ...
  module: StorageModule = request.app.state.storage
  await module.delete_files(user_guid, data.files)
  return RPCResponse(
    op=rpc_request.op,
    payload=data.model_dump(),
    version=rpc_request.version,
...
async def storage_files_set_gallery_v1(request: Request):
  ...
  module: StorageModule = request.app.state.storage
  await module.set_gallery(user_guid, data.name, data.gallery)
  return RPCResponse(
    op=rpc_request.op,
    payload=data.model_dump(),
    version=rpc_request.version,
...
async def storage_files_create_folder_v1(request: Request):
  ...
  module: StorageModule = request.app.state.storage
  await module.create_folder(user_guid, data.path)
  return RPCResponse(
    op=rpc_request.op,
    payload=data.model_dump(),
    version=rpc_request.version,
...
async def storage_files_move_file_v1(request: Request):
  ...
  module: StorageModule = request.app.state.storage
  await module.move_file(user_guid, data.src, data.dst)
  return RPCResponse(
    op=rpc_request.op,
    payload=data.model_dump(),
    version=rpc_request.version,
...
async def storage_files_rename_file_v1(request: Request):
  ...
  module: StorageModule = request.app.state.storage
  await module.rename_file(user_guid, data.old_name, data.new_name)
  return RPCResponse(
    op=rpc_request.op,
    payload=data.model_dump(),
    version=rpc_request.version,
...
async def storage_files_get_metadata_v1(request: Request):
  ...
  module: StorageModule = request.app.state.storage
  meta = await module.get_file_metadata(user_guid, data.name)
...
async def storage_files_delete_folder_v1(request: Request):
  ...
  module: StorageModule = request.app.state.storage
  await module.delete_folder(user_guid, data.path)
  return RPCResponse(
    op=rpc_request.op,
    payload=data.model_dump(),
    version=rpc_request.version,
...
async def storage_files_create_user_folder_v1(request: Request):
  ...
  module: StorageModule = request.app.state.storage
  await module.create_user_folder(user_guid, data.path)
  return RPCResponse(
    op=rpc_request.op,
    payload=data.model_dump(),
    version=rpc_request.version,
...
async def storage_files_get_usage_v1(request: Request):
  ...
  if user_guid is None:
    raise HTTPException(status_code=400, detail="Missing user GUID")
  module: StorageModule = request.app.state.storage
  usage = await module.get_usage(user_guid)
...
async def storage_files_report_file_v1(request: Request):
  ...
  module: StorageModule = request.app.state.storage
  await module.report_file(data.guid, data.name)
  return RPCResponse(
    op=rpc_request.op,
    payload=data.model_dump(),
    version=rpc_request.version,
...
async def storage_files_get_public_files_v1(request: Request):
  ...
  module: StorageModule = request.app.state.storage
  files = await module.list_public_files()
  items = [StorageFilesFileItem1(**f) for f in files]
  payload = StorageFilesFiles1(files=items)
  return RPCResponse(
    op=rpc_request.op,
...
async def storage_files_get_moderation_files_v1(request: Request):
  rpc_request, _, _ = await unbox_request(request)
  module: StorageModule = request.app.state.storage
  files = await module.list_flagged_for_moderation()
  items = [StorageFilesFileItem1(**f) for f in files]
  payload = StorageFilesFiles1(files=items)
  return RPCResponse(
    op=rpc_request.op,
